...
public class MainActivity extends AppCompatActivity {
    ...
    private final View.OnClickListener listener = new View.OnClickListener() {
        ...
        public void onClick(View view) {
            String searchText = search.getText().toString().trim(); // считывание введённого текста

            mapPoem = stringMapPoem(poem); // формирование мап подстрок поэмы

            String resultSearch = searchMapPoem(Integer.parseInt(searchText), mapPoem); // поиск String searchText в мап

            output.setText(resultSearch == null ? "Строки с такой длинной не найдено": "В поэме найдено " + resultSearch);
        }
    };

    // метод поиска информации в мап
    private String searchMapPoem(int enteredLength, Map<Integer, String> map) {

        StringBuilder result = new StringBuilder();
        int count = 0;

        for (Integer key : map.keySet()) { // перебор мап по ключам
            if (Objects.requireNonNull(map.get(key)).length() == enteredLength) { // если в строке с ключом key имеется фраза поиска, то
                count++; // счётчик найденных фраз
                result.append("\n\n").append(map.get(key)); // формирование строки найденных в поэме подстрок
            }
        }

        return count != 0 ? (count + " соответствий:" + result) : null;
    }
